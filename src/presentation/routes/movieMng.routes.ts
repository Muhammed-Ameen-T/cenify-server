...
router.post('/rate', verifyAccessToken,  authorizeRoles(['user']), (req, res, next) => movieMngController.submitRating(req, res, next));

router.post('/like', verifyAccessToken,authorizeRoles(['user']), (req, res, next) => movieMngController.submitRating(req, res, next));
