...
router.post('/create', verifyAccessToken, authorizeRoles(['user']), (req, res, next) =>
...
router.get(
  ...
  verifyAccessToken,
  authorizeRoles(['user']),
  (req, res, next) => bookingController.checkPaymentOptions(req, res, next),
);
router.get('/fetch', verifyAccessToken, authorizeRoles(['user', 'admin']), (req, res, next) =>
  bookingController.fetchBookings(req, res, next),
);
