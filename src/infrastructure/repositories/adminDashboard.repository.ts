...
export class AdminDashboardRepository implements IAdminDashboardRepository {
  async getDashboardData(
    ...
    try {
      ...
      // Base booking match
      const bookingMatch: any = { status: 'confirmed', 'payment.status': 'completed' };
  ...
  private async getSalesData(bookingMatch: any, period: string): Promise<SalesData[]> {
    let dateFormat: string;
  ...
  private async getTopTheaters(
    ...
    const currentStats = await BookingModel.aggregate([
      { $match: bookingMatch },
      {
        $lookup: {
          from: 'shows',
    ...
    const previousStats = await BookingModel.aggregate([
  ...
  private async getTopShows(bookingMatch: any): Promise<TopShow[]> {
    ...
    const hotShows = await BookingModel.aggregate([
      {
        $match: {
          status: 'confirmed',
      ...
      { $sort: { bookings: -1 } },
      { $limit: Math.ceil(0.25 * (await ShowModel.countDocuments())) }, // Top 25%
      {
        $group: {
          _id: null,
          hotShowIds: { $push: '$_id' },
    ...
    // Get top shows
    const shows = await BookingModel.aggregate([
      { $match: bookingMatch },
      {
        $lookup: {
          from: 'shows',
  ...
  private async getTheaterStatus(theaterMatch: any): Promise<TheaterStatus[]> {
    const statusCounts = await TheaterModel.aggregate([
      { $match: theaterMatch },
      {
        $group: {
          _id: '$status',
    ...
    const result: TheaterStatus[] = Object.keys(statusMap).map((name) => ({
